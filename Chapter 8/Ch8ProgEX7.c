...
************************************************************
Modify programming exercise 8 from chapter 7 so that the menu
choices are labeled by characters instead of by numbers; use
q instead of 5 as the cue to terminate input.
*/
#include <stdio.h>
...
int main(void)
{
    ...
    while (pay_choice != 'q')
    {
        ...
        switch (pay_choice = getchar())
        {
            case 'a' : pay_rate = EARNRATE1;
                       break;
            case 'b' : pay_rate = EARNRATE2;
                       break;
            case 'c' : pay_rate = EARNRATE3;
                       break;
            case 'd' : pay_rate = EARNRATE4;
                       break;
            case 'q' : continue;
            default  : printf("Incorrect choice; choose a letter from a to d or q to quit.\n");
                     continue;
        }
